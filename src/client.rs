...
pub(crate) async fn start_client() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let connection = Text::new("Please enter the ip address and port you would like to connect to:").prompt().unwrap();
    let mut socket = TcpStream::connect(connection).await;
    match socket {
        Ok(mut socket) => {
